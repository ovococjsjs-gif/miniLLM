#!/usr/bin/env python3
"""Build the public-API llama.cpp neural output-adapter tool."""
...
def concrete_library(directory: Path, stem: str) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(
            f"cannot find concrete {stem} shared library in {directory}"
        )
...
def main() -> None:
    ...
    parser.add_argument("--source", default="native/qwen35_output_adapter.cpp")
    parser.add_argument(
        "--binary", default=".cache/qwen35-output-adapter/qwen35-output-adapter"
    )
    parser.add_argument("--output", default="results/qwen35_output_adapter_build.json")
    ...
    report = {
        "schema_version": 1,
        "role": "neural-output-adapter-runtime-build",
