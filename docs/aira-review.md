...
> **Исторический документ, частично отменён.** Пользователь подтвердил, что AIra — его
> собственный репозиторий, и явно разрешил перенос кода и идей. Консервативное решение
> «не переносить AIra и оставить обычный LLM baseline главным» больше не действует.
> Актуальный полный аудит и план завершения: [`aira-v2-audit.md`](aira-v2-audit.md).

Дата первого разбора: 2026-08-20. Рассмотрена ветка
[`arena/019fcef3-aira`](https://github.com/ovososjdjd-boop/AIra/tree/arena/019fcef3-aira),
локальный снимок `1bc5975`.

## Происхождение

AIra принадлежит пользователю; код и идеи разрешено свободно использовать в miniLLM.
При переносе сохраняются ссылка на исходный commit, экспериментальные границы и
отрицательные результаты. Числа AIra не считаются автоматическим доказательством на новом
стеке: значительная их часть получена на символьных или синтетических потоках.
...
## Историческое решение (отменено)

Первый разбор оставлял AIra только источником изолированных opt-in гипотез. После
подтверждения происхождения и полного повторного аудита это решение отменено. Attention
остаётся измерительным quality-control, но основная исследовательская линия теперь —
AIra-v2: trigger shelf, bounded associative memory, residual neural core и локальное
continual learning по воротам из `aira-v2-audit.md`.
