...
> **Историческая conventional-LLM ветка.** После аудита исходной AIra ближайший путь и
> критерии решения заменены программой AIra-v2 в
> [`aira-v2-audit.md`](aira-v2-audit.md), разделы 5–8. Обычные Attention/Edge модели
> ниже сохраняются как matched controls; указанные L2–L4 scaling runs сейчас
> приостановлены. Этот документ не является текущей очередью запуска.
